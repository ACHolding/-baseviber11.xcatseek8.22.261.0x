#!/usr/bin/env python3
"""catseekr1-base-1b 1.x — offline bilingual RAM-only BitNet LLM on the Kimi K3s engine.

catseekr1-base-1b materializes a real ~1 billion-parameter BitNet b1.58 Transformer
in RAM when the program starts (files=off). Boot target ≈ **0.2 seconds** via a
sparse 2-bit bank (resident STE pages only; virtual ~1B slots). The inference
...
GUI/CLI, catseekr1-base-1b Code, catseekr1-base-1b Build (Grok Build–style apps/games), and
self-tests live in this file.

Real BitNet (Microsoft BitNet b1.58) at ~1B:
    ...
    * ~1B ternary slots; sparse pages materialize the live STE slice only
...
try:
    ...
    raise SystemExit(
        "catseekr1-base-1b requires NumPy for real in-memory BitNet training. "
        "No model checkpoint is required."
    ) from exc


APP_NAME = "catseekr1-base-1b"
APP_VERSION = "1B.1"
MODEL_ID = "catseekr1-base-1b-bitnet-b1.58-kimi-k3s-dspark-deepseek-r1-ram"
FILES_MODE = "off"
ENGINE_ID = "kimi-k3s"
...
BITNET_EPS = 1e-5
# Deep-narrow BitNet Transformer ≈ 1.0B ternary weights (files=off, packed).
BITNET_1B_N_LAYERS = 318
BITNET_1B_D_MODEL = 512
BITNET_1B_D_FF = 2048
BITNET_1B_N_HEADS = 8
...
class ModelConfig:
    ...
    embedding_dim: int = BITNET_1B_D_MODEL
    hidden_dim: int = BITNET_1B_D_FF
    d_model: int = BITNET_1B_D_MODEL
    d_ff: int = BITNET_1B_D_FF
    n_layers: int = BITNET_1B_N_LAYERS
    n_heads: int = BITNET_1B_N_HEADS
    mod_capacity: int = 2
    train_layers: int = 1
...
BOOT_DIALOGUES: tuple[tuple[tuple[str, ...], str], ...] = (
    (("hello", "hi", "hey catseekr1-base-1b", "good morning"),
     "Hello! I am catseekr1-base-1b, running locally from RAM. What are we building?"),
    (("who are you", "what model are you", "tell me about yourself"),
     "I am catseekr1-base-1b, a compact next-token language model trained in memory when this program starts."),
    ...
    (("what is dual residual ternary", "explain your new math", "what is dual residual ternary"),
     "Earlier builds used a dual-residual ternary shortcut. This build is real BitNet b1.58 BitLinear on the Kimi K3s engine: absmean ternary matrices, A8 activations, KDA/AttnRes/LatentMoE, and an integer ternary matmul kernel."),
    ...
    (("what is tokenization", "explain tokens", "how do you tokenize text"),
     "Tokenization converts text into vocabulary identifiers. catseekr1-base-1b uses compact word, punctuation, newline, and control tokens."),
    (("write Python hello world", "make a Python hello program", "Python print example"),
     "Here is Python code:\n```\nprint(\"Hello from catseekr1-base-1b\")\n```"),
    ...
    (("make a game", "game development plan", "how do I build a game"),
     "Use catseekr1-base-1b Build: say /build make a snake game — I write a playable single-file HTML game under builds/ (Grok Build–style, local, files=off for model weights)."),
    (("build an app", "make an app", "grok build", "catseek build"),
     "catseekr1-base-1b Build turns a plain-language idea into a working local HTML/JS app or game under builds/. Try /build todo app or python3 catseekr1-base-1b.py --build."),
    ...
    (("thank you", "thanks", "nice work"),
     "You are welcome! catseekr1-base-1b is ready for the next task."),
    (("goodbye", "bye", "see you later"),
     "Goodbye! The conversation can end while the in-memory model remains ready."),
    (("你好", "嗨", "早上好", "晚上好"),
     "你好！我是 catseekr1-base-1b。检测到中文后，我会自动使用中文回答。"),
    (("你是谁", "你是什么模型", "请介绍一下自己"),
     "我是 catseekr1-base-1b，一个启动时在内存中训练的本地下一词元语言模型。"),
    ...
    (("什么是分词", "解释词元", "文本怎样变成数字"),
     "分词把文字转换成词表编号。catseekr1-base-1b 对英文使用单词和标点，对中文使用汉字和标点。"),
    ...
    (("谢谢", "感谢你", "做得好"),
     "不客气！catseekr1-base-1b 已准备好处理下一个任务。"),
    (("再见", "拜拜", "回头见"),
...
# When catseekr1-base-1b lacks grounded knowledge, the correct learned behavior is to say
...
class TernaryBank:
    """Sparse 2-bit BitNet bank: virtual ~1B slots, resident STE pages only.

    Boot stays ≤0.2s because we never allocate the full ~250MB packed buffer up front.
...
class InMemoryTernaryLM:
    """Real BitNet b1.58 ~1B LLM on the Kimi K3s engine (W1.58A8), files=off.

    Default topology lands at ~1.0B ternary slots:
      n_layers=318, d_model=512, d_ff=2048, n_heads=8
    ...
    def model_card(self) -> dict[str, object]:
        ...
        return {
            ...
            "model_id": MODEL_ID,
            "version": APP_VERSION,
            "kind": "experimental BitNet b1.58 W1.58A8 Kimi K3s MoD runtime with a ~1B-slot sparse bank",
            "files": FILES_MODE,
            "engine": ENGINE_ID,
            ...
            "normal_chat_dispatch": "CatSeekR1Base1B.reply hybrid router",
            ...
            "real_parameter_target": 1_000_000_000,
            "honesty": (
                "A ~1B-slot BitNet b1.58 bank is held as sparse ternary pages in RAM "
    ...
    def self_test(self) -> dict[str, object]:
        ...
        tests = {
            ...
            "parameter_count_is_about_1b": 900_000_000 <= params <= 1_200_000_000,
...
class CatSeekR1Base1BCodeTools:
...
class CatSeekR1Base1BCodeAgent:
    """Local Claude Code fork powered by the real BitNet ~1B LM (files=off weights)."""

    def __init__(self, engine: "CatSeekR1Base1B", root: Optional[str] = None):
        self.engine = engine
        self.tools = CatSeekR1Base1BCodeTools(root)
    ...
    def run(self, prompt: str) -> Reply:
        ...
        if actions:
            ...
            footer = (
                f"\n\ncatseekr1-base-1b Code · {ok_n}/{len(results)} tools ok · "
                f"brain {MODEL_ID} · model files={FILES_MODE}"
            )
            text = body + footer
            route = f"catseekr1-base-1b-code:{len(actions)}tools"
        else:
            # No tool plan: fall through the full catseekr1-base-1b stack (exact/semantic/BitNet).
            neural = self.engine.reply(prompt)
            text = neural.text
            route = f"catseekr1-base-1b-code:{neural.route}"
...
class CatSeekR1Base1BBuildMode:
    ...
    BUILD_RE = re.compile(
        ...
        r"|/build\b|grok\s*build|catseekr1-base-1b\s*build|catseek\s*build",
        re.IGNORECASE,
    )
...
<title>{title} · catseekr1-base-1b Build</title>
<style>
...
<header>
  <h1>{title}</h1>
  <p>catseekr1-base-1b Build · local HTML artifact · BitNet brain files=off</p>
</header>
<main>
{body}
</main>
...
"""
            ...
            script = r"""
const list=document.getElementById('list'),items=JSON.parse(localStorage.getItem('catseekr1-base-1b-todo')||'[]');
function save(){localStorage.setItem('catseekr1-base-1b-todo',JSON.stringify(items));}
...
"""
            ...
            body = f'<div class="card"><h2>Ship the idea</h2><p class="muted">Prompt: {prompt[:180].replace("<","&lt;")}</p><p>This landing page was generated by catseekr1-base-1b Build from your description. Replace the copy, drop in art, and publish anywhere static HTML hosts.</p><div class="row"><a class="btn" href="#get">Get started</a><button onclick="alert(\'Local catseekr1-base-1b Build CTA\')">Live demo</button></div></div><div class="card" id="get" style="margin-top:14px"><h3>Why catseekr1-base-1b Build</h3><ul><li>Plain-language → working HTML</li><li>BitNet brain stays files=off</li><li>Iterate with /build again</li></ul></div>'
            script = "/* static landing */"
        else:
            body = f'<div class="card"><h2>Interactive canvas</h2><p class="muted">{prompt[:200].replace("<","&lt;")}</p><canvas id="c" width="480" height="320"></canvas><p class="muted">Click to paint · generated by catseekr1-base-1b Build</p></div>'
            script = r"""
...
"""
    ...
    def build(self, prompt: str) -> Reply:
        ...
        text = (
            f"catseekr1-base-1b Build created **{title}** ({kind}).\n\n"
            f"Saved: `{path}`\n"
            f"Open: {uri}\n\n"
        ...
        return Reply(
            text=text,
            route=f"catseekr1-base-1b-build:{kind}",
            elapsed_ms=(time.perf_counter() - started) * 1000.0,
    ...
    def self_test(self) -> dict[str, object]:
        ...
        if self.last_path and self.last_path.exists():
            try:
                ...
                pass
        return {
            "ok": ok and reply.route.startswith("catseekr1-base-1b-build:"),
            "route": reply.route,
...
class CatSeekR1Base1B:
    ...
    def __init__(
        ...
        self.code_agent = CatSeekR1Base1BCodeAgent(self)
        self.build_mode = CatSeekR1Base1BBuildMode()
    ...
    def model_card(self) -> dict[str, object]:
        ...
        card.update({
            "runtime": "catseekr1-base-1b cognitive inference runtime",
            "reasoning_tools": [
                ...
                "catseekr1-base-1b Code (Claude Code fork): Read/Write/Edit/Bash/Glob/Grep/LS",
            ],
    ...
    def self_test(self) -> dict[str, object]:
        ...
        return {
            ...
            "catseekr1_base_1b_code": code,
            "catseekr1_base_1b_build": build,
            "runtime": {
                "files": FILES_MODE,
                "network_required": False,
                "boot_elapsed_s": round(getattr(self, "boot_elapsed_s", 0.0), 4),
    ...
    def _command(self, prompt: str) -> Optional[Reply]:
        ...
        if lower in {"/help", "/?"}:
            text = (
                ...
                "- `/code PROMPT` — catseekr1-base-1b Code (Claude Code fork) on BitNet 1B\n"
                "- `/build PROMPT` — catseekr1-base-1b Build (Grok Build–style apps/games)\n"
                "- Complex prompts auto-route through DeepSeek R1-style think/answer on BitNet + DSpark (files=off)\n\n"
                "Ordinary messages use exact tools, semantic memory, prompt-grounded handlers, Build mode for apps/games, and quality-gated BitNet decoding. The ~1B value describes packed architecture slots, not pretrained frontier knowledge. Use --code or --build for dedicated REPLs."
            )
        ...
        if lower == "/trace":
            if not self.model.last_trace:
                text = "No catseekr1-base-1b generation trace exists yet."
            else:
                rows = [
                    {
                        "step": step.index,
        ...
        if lower == "/clear":
            self.clear()
            return Reply("catseekr1-base-1b conversation context cleared.", "control:/clear", (time.perf_counter() - started) * 1000)
        if lower.startswith("/train"):
            ...
            text = (
                f"catseekr1-base-1b trained for {report.steps} additional RAM-only steps. "
        ...
        if lower.startswith("/candidates"):
            parts = raw.split()
            if len(parts) == 1:
                text = f"catseekr1-base-1b uses up to {self.candidate_count} verified candidates."
            else:
                try:
                    value = int(parts[1])
                except ValueError:
                ...
                text = f"catseekr1-base-1b candidate count set to {self.candidate_count}."
            return Reply(text, "control:/candidates", (time.perf_counter() - started) * 1000)
        if lower.startswith("/temperature"):
            parts = raw.split()
            if len(parts) == 1:
                text = f"catseekr1-base-1b temperature is {self.temperature:.2f}."
            else:
                try:
                    value = float(parts[1])
                except ValueError:
                ...
                text = f"catseekr1-base-1b temperature set to {self.temperature:.2f}."
            return Reply(text, "control:/temperature", (time.perf_counter() - started) * 1000)
        if lower.startswith("/code"):
            task = raw[len("/code"):].strip()
            if not task:
                return Reply(
                    "Usage: /code PROMPT   or launch `python3 catseekr1-base-1b.py --code`",
                    "control:/code",
                    (time.perf_counter() - started) * 1000,
                )
        ...
        if lower.startswith("/build"):
            ...
            if not task:
                return Reply(
                    "Usage: /build PROMPT   e.g. /build snake game\nOr: python3 catseekr1-base-1b.py --build",
                    "control:/build",
                    (time.perf_counter() - started) * 1000,
                )
            reply = self.build_mode.build(task)
            return Reply(reply.text, reply.route, (time.perf_counter() - started) * 1000.0, reply.tokens, reply.tokens_per_second)
        return Reply("Unknown catseekr1-base-1b command. Use /help.", "control:unknown", (time.perf_counter() - started) * 1000)
    ...
    def reply(self, prompt: str, on_token: Optional[Callable[[str], None]] = None) -> Reply:
        ...
        if CatSeekR1Base1BBuildMode.wants(prompt):
...
class CatSeekR1Base1BGUI:
    ...
    def __init__(self, root: tk.Tk, engine: CatSeekR1Base1B):
        self.root = root
        self.engine = engine
    ...
    def _build(self) -> None:
        ...
        base.add_command(label="New catseekr1-base-1b chat", command=self._new_chat)
        base.add_command(label="catseekr1-base-1b model card", command=lambda: self._submit("/model"))
        base.add_command(label="catseekr1-base-1b inference self-test", command=lambda: self._submit("/selftest"))
        base.add_separator()
        base.add_command(label="Exit catseekr1-base-1b", command=root.destroy)
        ...
        tk.Label(
            name,
            text=(
                f"Real BitNet b1.58 ~1B · {ENGINE_NAME} · {self.engine.model.parameter_count:,} params · "
        ...
        self.status = tk.Label(header, text="catseekr1-base-1b inference ready", bg=self.PANEL, fg=self.BLUE_2, font=self._font(10))
        ...
        tk.Button(
            sidebar,
            text="+ New catseekr1-base-1b chat",
        ...
        self._append("CATSEEKR1-BASE-1B\n", "title")
        self._append(
    ...
    def _new_chat(self) -> None:
        ...
        if self.busy:
            ...
            self.status.config(text="catseekr1-base-1b new chat ready")
        self.entry.focus_set()
    ...
    def _submit(self, forced: Optional[str] = None) -> None:
        ...
        self.status.config(text="catseekr1-base-1b computing next tokens…")
        ...
        try:
            while True:
                ...
                if kind == "done" and is_current:
                    ...
                    self.status.config(text=f"catseekr1-base-1b ready · {reply.tokens_per_second:.1f} tok/s")
                elif kind == "error" and is_current:
                    self._append("ERROR\n" + str(payload) + "\n", "error")
                    self.status.config(text="catseekr1-base-1b error")
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--code", action="store_true", help="catseekr1-base-1b Code REPL (Claude Code fork on BitNet 1B)")
    parser.add_argument("--build", action="store_true", help="catseekr1-base-1b Build REPL (Grok Build–style apps/games)")
    ...
    parser.add_argument(
        "--vibe-check",
        action="store_true",
        help="audit embedded training data and arbitrary-prompt coverage without allocating the 1B bank",
    )
...
def terminal(engine: CatSeekR1Base1B, one_prompt: Optional[str] = None) -> int:
...
def code_terminal(engine: CatSeekR1Base1B, one_prompt: Optional[str] = None, cwd: Optional[str] = None) -> int:
    agent = CatSeekR1Base1BCodeAgent(engine, root=cwd)
...
def build_terminal(engine: CatSeekR1Base1B, one_prompt: Optional[str] = None, cwd: Optional[str] = None) -> int:
    builder = CatSeekR1Base1BBuildMode(root=cwd)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    profiles = {
        # All profiles keep the real ~1B BitNet + Kimi K3s trunk; default boot ≤0.2s.
        "fable5": dict(
            context_tokens=24,
            mod_capacity=2,
            train_layers=1,
    ...
    config = ModelConfig(
        ...
        embedding_dim=BITNET_1B_D_MODEL,
        hidden_dim=BITNET_1B_D_FF,
        d_model=BITNET_1B_D_MODEL,
        d_ff=BITNET_1B_D_FF,
        n_layers=BITNET_1B_N_LAYERS,
        n_heads=BITNET_1B_N_HEADS,
        mod_capacity=profile["mod_capacity"],
        train_layers=profile["train_layers"],
        dense_init=False,
        reasoning_passes=profile["mod_capacity"],
        max_reasoning_passes=BITNET_1B_N_LAYERS,
    ...
    if show_startup_progress:
        print(
            f"Materializing ~1B BitNet b1.58 + {ENGINE_NAME} in RAM (files=off, {config.speed_target}): "
    ...
    try:
        engine = CatSeekR1Base1B(config, progress=progress)
    ...
    if not (args.self_test or args.model_card):
        print(
            ...
            f"files={FILES_MODE} · BitNet b1.58 ~1B slots",
            file=sys.stderr,
            flush=True,
        )
    if args.self_test:
    ...
    root = tk.Tk()
    CatSeekR1Base1BGUI(root, engine)
    root.mainloop()
    return 0
